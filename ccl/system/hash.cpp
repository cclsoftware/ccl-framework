...
//
// Filename    : ccl/system/hash.cpp
// Description : Hash Function found at http://burtleburtle.net/bob/hash/doobs.html
//
//************************************************************************************************

#include "ccl/public/systemservices.h"

typedef CCL::uint8 uint8_t;
typedef CCL::uint16 uint16_t;
typedef CCL::uint32 uint32_t;

static uint32_t hashword (uint32_t *k, size_t length, uint32_t initval);
static uint32_t hashlittle (void* key, size_t length, uint32_t initval);
static uint32_t hashbig (void* key, size_t length, uint32_t initval);
...
CCL_EXPORT uint32 CCL_API System::CCL_ISOLATED (Hash) (const void* key, uint32 length, uint32 initialValue)
{
#if CCL_NATIVE_BYTEORDER == CCL_LITTLE_ENDIAN
	return hashlittle (const_cast<void*> (key), length, initialValue);
#else
	return hashbig (const_cast<void*> (key), length, initialValue);
#endif
}
...
uint32_t hashlittle (void* key, size_t length, uint32_t initval)
{
  ...
  if (HASH_LITTLE_ENDIAN && !((((uint8_t *)key)-(uint8_t *)nullptr) & 0x3)) {
  ...
  } else if (HASH_LITTLE_ENDIAN && !((((uint8_t *)key)-(uint8_t *)nullptr) & 0x1)) {
...
uint32_t hashbig (void* key, size_t length, uint32_t initval)
{
  ...
  if (HASH_BIG_ENDIAN && !((((uint8_t *)key)-(uint8_t *)nullptr) & 0x3)) {
    uint32_t *k = (uint32_t*)key;                             /* read 32-bit chunks */

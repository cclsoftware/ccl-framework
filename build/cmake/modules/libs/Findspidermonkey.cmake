...
set (spidermonkey_version "147.0.3")
...
set (spidermonkey_downloaded OFF)
...
if (NOT EXISTS "${spidermonkey_platform_dir}")
	message (STATUS "Restoring SpiderMonkey ${spidermonkey_version} libraries from cache...")
	ccl_restore_from_cache (spidermonkey FILE_NAME "${spidermonkey_platform_zip_name}" DESTINATION "${spidermonkey_download_path}")

	if (NOT EXISTS "${spidermonkey_platform_zip_path}")
		message (STATUS "Downloading SpiderMonkey ${spidermonkey_version} libraries...")
		download_from_github_release (cclsoftware/firefox "${spidermonkey_release_name}" "${spidermonkey_platform_zip_name}" "${spidermonkey_download_path}" PUBLIC)

		if (NOT EXISTS "${spidermonkey_platform_zip_path}")
			message (FATAL_ERROR "Failed to download SpiderMonkey libraries.")
			return ()
		endif ()

		set (spidermonkey_downloaded ON)
	endif ()

	# extract the download and remove archive
	message (STATUS "Extracting SpiderMonkey ${spidermonkey_version} libraries...")
	file (ARCHIVE_EXTRACT INPUT "${spidermonkey_platform_zip_path}" DESTINATION "${spidermonkey_download_path}" TOUCH)
	if (${spidermonkey_downloaded} AND EXISTS "${spidermonkey_platform_dir}")
		ccl_add_to_cache (spidermonkey "${spidermonkey_platform_zip_path}")
	endif ()
	file (REMOVE "${spidermonkey_platform_zip_path}")
	if (NOT EXISTS "${spidermonkey_platform_dir}")
		message (FATAL_ERROR "Failed to extract SpiderMonkey libraries from downloaded ${spidermonkey_platform_zip_name}.")
		return ()
	endif ()
endif ()
